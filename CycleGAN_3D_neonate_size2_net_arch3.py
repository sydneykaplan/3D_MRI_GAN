...
# Help functions
    ...
    def cycle_loss(self, y_true, y_pred):
        ...
        print("mae is: ", loss1)
        loss2 = tf.multiply(tf.divide(tf.log(tf.reduce_max(y_pred)), tf.log(10.0)), 20.0) + tf.multiply(tf.divide(tf.log(loss1), tf.log(10.0)), 10.0)
        print("psnr is: ", loss2)
        loss = loss1 - tf.divide(loss2, 10)
        return loss
...
# Save and load
    ...
    def saveModel(self, model, epoch):
        ...
        directory = os.path.join('/outdata', 'saved_models', self.date_time)
        if not os.path.exists(directory):
            os.makedirs(directory)
    ...
    def load_model_and_weights(self, model):
        path_to_model = os.path.join('/outdata', 'generate_images', 'models', '{}.json'.format(model.name))
        path_to_weights = os.path.join('/outdata', 'generate_images', 'models', '{}.hdf5'.format(model.name))
    ...
    def load_model_and_generate_synthetic_images(self):
        ...
        def save_image(image, name, domain):
            if self.channels == 1:
                image = image[:, :, :, 0]
            nib.save(nib.Nifti1Image(image, np.eye(4)), os.path.join('/outdata', 'generate_images', 'synthetic_images', domain, name))
